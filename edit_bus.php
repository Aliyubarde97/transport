...
$busID=$_GET['id'];
$find_bus=mysqli_query($conn, "select * from buses where id='$busID'");
$finded_bus=mysqli_fetch_array($find_bus);
...
<html lang="en">
    <head>
		...
		<body>
				...
				<h1 align="center">Update Bus Info</h1>	
				<hr>
				<div style="float:left; margin-left:400px">
				<a href="buses.php"><button type="submit" class="btn btn-danger" name="submit">Back</button></a>
				</div>
                <br><br>
					...
					<div class="col-sm-3">
					<input type="text" name="bnumber" value="<?php echo $finded_bus['Bus_No'] ?>" class= "form-control" required placeholder="Bus number">
					</div>
					</div>
					...
					<div class="col-sm-3">
					<input type="text" name="seat" value="<?php echo $finded_bus['number_seat'] ?>" class= "form-control" required placeholder="number of Seat">
					</div>
					</div>
					<div class="form-group">
...
if(isset($_POST['submit']))
{
	...
	$insert=mysqli_query($conn, "update buses set Bus_no='$bnumber', city='$city', number_seat='$seat', status='$status', depart_time='$dtime' where id='$busID'");
	if($insert)
	{
		...
		echo "Unable to Update Bus info ".mysqli_error($conn);
	}
